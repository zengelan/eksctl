...
eksctl: $(call gofiles,go list -f '{{join .Deps "\n"}}' ./cmd/...) ## Build main binary
...
APIS_SOURCES=$(shell ls ./pkg/apis/eksctl.io/v1alpha5/* | grep -v zz_ | grep -v _test)
pkg/apis/eksctl.io/v1alpha5/zz_generated.deepcopy.go: $(APIS_SOURCES) .license-header go.mod ## Generate Kubernetes API helpers
...
site/content/usage/20-schema.md: cmd/schema/generate.go go.mod $(APIS_SOURCES)
	time go run ./cmd/schema/generate.go $@

$(AWS_SDK_MOCKS): pkg/eks/mocks/mocks.go go.mod
